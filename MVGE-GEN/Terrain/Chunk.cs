...
namespace MVGE_GEN.Terrain
{
    ...
    public class Chunk
    {
        ...
        private readonly int dimX; // cached dimensions to avoid repeated settings lookups
        ...
        public void GenerateInitialChunkData()
        {
            ...
            for (int x = 0; x < maxX; x++)
            {
                for (int z = 0; z < maxZ; z++)
                {
                    ...
                    int sx = x >> SECTION_SHIFT; int sz = z >> SECTION_SHIFT;
                    for (int sy = 0; sy <= maxNonAirLocalY >> SECTION_SHIFT; sy++)
                    {
                        if (sections[sx, sy, sz] == null) sections[sx, sy, sz] = new ChunkSection();
                    }

                    if (hasStone)
                    {
                        for (int ly = 0; ly <= stoneLocalTop && ly < maxY; ly++) SetBlockLocal(x, ly, z, (ushort)BaseBlockType.Stone);
                    }
                    if (hasSoil)
                    {
        ...
        public ushort GenerateInitialBlockData(int lx, int ly, int lz, int columnHeight)
        {
            ...
            if (type == EMPTY && currentHeight < columnHeight + soilModifier) type = (ushort)BaseBlockType.Soil;
            return type;
        }
        ...
        public static float[,] GenerateHeightMap(long seed, int chunkBaseX, int chunkBaseZ)
        {
            ...
            int maxX = GameManager.settings.chunkMaxX; int maxZ = GameManager.settings.chunkMaxZ;
            float[,] heightmap = new float[maxX, maxZ];
            float scale = 0.005f; float minHeight = 1f; float maxHeight = 200f;
            for (int x = 0; x < maxX; x++)
                for (int z = 0; z < maxZ; z++)
                {
        ...
        { sx = lx >> SECTION_SHIFT; sy = ly >> SECTION_SHIFT; sz = lz >> SECTION_SHIFT; ox = lx & SECTION_MASK; oy = ly & SECTION_MASK; oz = lz & SECTION_MASK; }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ushort GetBlockLocal(int lx, int ly, int lz)
        {
            // single unsigned bounds check each axis
            if ((uint)lx >= (uint)dimX || (uint)ly >= (uint)dimY || (uint)lz >= (uint)dimZ) return EMPTY;
            int sx = lx >> SECTION_SHIFT; int sy = ly >> SECTION_SHIFT; int sz = lz >> SECTION_SHIFT;
            var sec = sections[sx, sy, sz]; if (sec == null) return EMPTY;
            int ox = lx & SECTION_MASK; int oy = ly & SECTION_MASK; int oz = lz & SECTION_MASK;
            // Inline fast block fetch if SectionUtils has overhead; else call util.
            return SectionUtils.GetBlock(sec, ox, oy, oz);
        }

        public void SetBlockLocal(int lx, int ly, int lz, ushort blockId)
        {
            if ((uint)lx >= (uint)dimX || (uint)ly >= (uint)dimY || (uint)lz >= (uint)dimZ) return;
